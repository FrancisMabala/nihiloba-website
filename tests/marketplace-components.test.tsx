...
import WenzeStoreRoute from "../app/(default)/shida/wenze/[store]/page";
import WenzeProductRoute from "../app/(default)/shida/wenze/products/[product]/page";
...
describe("marketplace presentation", () => {
  ...
  it("routes fixed-price cards through the shared cart and variant products through selection",()=>{const product:WenzeProduct={public_ref:"P1",slug:"shoe",name:"Shoe",description:null,category:"shoes_accessories",price:"20",currency:"USD",price_negotiable:false,available_stock:2,has_variants:false,variant_type:null,variants:[],images:[],public_detail_url:"https://nihiloba.com/shida/wenze/products/shoe",buy_url:"https://wa.me/simple-product",store:null,fulfillment:{methods:["pickup"],delivery:null}};const simple=renderToStaticMarkup(<ProductCard p={product} l="en"/>),variant=renderToStaticMarkup(<ProductCard p={{...product,has_variants:true,variant_type:"shoe_size",variants:[{public_ref:"V1",label:"42",variant_type:"shoe_size",stock_quantity:1,available_stock:1,is_available:true,buy_url:"https://wa.me/variant-42"}]}} l="fr"/>);expect(simple).toContain("Add to cart");expect(simple).toContain("Buy now");expect(simple).not.toContain("https://wa.me/simple-product");expect(variant).toContain("Voir le produit");expect(variant).not.toContain("simple-product");expect(variant).not.toContain("variant-42")});
  ...
  it("uses structured public locations for online-and-physical stores",async()=>{const base={public_ref:"WNZ-LOCATION",slug:"location-shop",name:"Location shop",description:"Public store",category:null,store_type:"online_and_physical",country_code:"CD",city:"Kinshasa",area:"Generic Zone",commune:"Lemba",quartier:"Mbanza",address:"Avenue 1",landmark:"Market",products:[],public_detail_url:"https://nihiloba.com/shida/wenze/location-shop",whatsapp_url:null,fulfillment:{methods:["pickup"],delivery:null},seller_phone:"private"};vi.stubGlobal("fetch",vi.fn().mockResolvedValueOnce(new Response(JSON.stringify(base))).mockResolvedValueOnce(new Response(JSON.stringify({...base,public_ref:"WNZ-GOMA",slug:"goma-shop",city:"Goma",area:"Centre",commune:null,quartier:null,address:null,landmark:null}))));const kinshasa=renderToStaticMarkup(await WenzeStorePage({l:"fr",id:"kinshasa-location-render-test"})),goma=renderToStaticMarkup(await WenzeStorePage({l:"en",id:"goma-location-render-test"}));expect(kinshasa).toContain("Boutique en ligne et physique");expect(kinshasa).toContain("Lemba, Kinshasa");expect(kinshasa).toContain("Mbanza");expect(kinshasa).toContain("Avenue 1");expect(kinshasa).toContain("Market");expect(kinshasa).not.toContain("Generic Zone");expect(goma).toContain("Centre, Goma");expect(goma).toContain("Area");expect(goma).not.toContain("private")});
  it("puts an image-first product grid before detailed store information",async()=>{const product={public_ref:"WNP-SHOE",slug:"brown-shoe",name:"Brown moccasin",description:"A long product description that must stay off the store card.",category:"shoes_accessories",price:"200",currency:"USD",price_negotiable:false,available_stock:1,has_variants:false,variant_type:null,variants:[],images:[{url:"https://res.cloudinary.com/dbrxpvmzp/image/upload/wenze/shoe.jpg",alt:"Brown moccasin",display_order:1}],public_detail_url:"https://nihiloba.com/shida/wenze/products/brown-shoe",buy_url:"https://wa.me/exact-product",fulfillment:{methods:["pickup","delivery"],delivery:{fee_type:"fixed",fee:"5000",currency:"CDF",areas:["Limete","Lemba"]}}};const store={public_ref:"WNZ-SHOP",slug:"sapologie",name:"SAPOLOGIE",description:"Quality shoes for everyday wear.",category:"shoes_accessories",store_type:"online_and_physical",country_code:"CD",city:"Kinshasa",area:"Generic Zone",commune:"Lemba",quartier:"Mbanza",address:"Avenue 1",landmark:"Market",products:[product,{...product,public_ref:"WNP-SHOE-2",slug:"black-shoe",name:"Black moccasin",images:[],buy_url:"https://wa.me/exact-product-2"}],public_detail_url:"https://nihiloba.com/shida/wenze/sapologie",whatsapp_url:"https://wa.me/exact-store",fulfillment:product.fulfillment,seller_phone:"private",internal_id:77};vi.stubGlobal("fetch",vi.fn().mockResolvedValue(new Response(JSON.stringify(store))));const html=renderToStaticMarkup(await WenzeStorePage({l:"en",id:"sapologie-commerce-layout-test"}));expect(html).toContain("SAPOLOGIE");expect(html).toContain("Online and physical shop");expect(html).toContain("Shoes &amp; accessories");expect(html).toContain("Lemba, Kinshasa");expect(html).toContain("Pickup available");expect(html).toContain("Delivery: 5,000 CDF");expect(html).toContain("<h2>Products<span> (2)</span></h2>");expect(html).toContain("Brown moccasin");expect(html).toContain("200 USD");expect(html).toContain("Fixed price");expect(html).toContain("1 available");expect(html).toContain("Add to cart");expect(html).not.toContain("https://wa.me/exact-product");expect(html).toContain("Image unavailable");expect(html).not.toContain("A long product description");expect(html).not.toContain("seller_phone");expect(html).not.toContain("internal_id");expect(html).not.toContain("private");expect(html.indexOf("Brown moccasin")).toBeLessThan(html.indexOf("Store information"));expect(html.indexOf("Brown moccasin")).toBeLessThan(html.indexOf("Communes served"));expect(html).toContain("Limete");expect(html).toContain("https://wa.me/exact-store")});

  it("labels online-only stores and never presents their fields as a physical address",async()=>{const store={public_ref:"WNZ-ONLINE",slug:"online-shop",name:"Boutique Web",description:null,category:null,store_type:"online_only",country_code:"CD",city:"Kinshasa",area:"PRIVATE AREA",commune:"PRIVATE COMMUNE",quartier:"PRIVATE QUARTIER",address:"PRIVATE ADDRESS",landmark:"PRIVATE LANDMARK",products:[],public_detail_url:"https://nihiloba.com/shida/wenze/online-shop",whatsapp_url:"https://wa.me/123",order_url:"https://api.nihiloba.com/go/online-order",fulfillment:{methods:["delivery"],delivery:{fee_type:"free",fee:null,currency:null,areas:["Gombe"]}},owner_phone:"PRIVATE PHONE",owner_id:99};vi.stubGlobal("fetch",vi.fn().mockResolvedValue(new Response(JSON.stringify(store))));const html=renderToStaticMarkup(await WenzeStorePage({l:"fr",id:"online-only-render-test"}));expect(html).toContain("Boutique en ligne");expect(html).toContain("Livraison gratuite");expect(html).toContain("Gombe");expect(html).toContain("https://api.nihiloba.com/go/online-order");for(const hidden of ["PRIVATE AREA","PRIVATE COMMUNE","PRIVATE QUARTIER","PRIVATE ADDRESS","PRIVATE LANDMARK","PRIVATE PHONE","owner_id","Carte","Itinéraire","Retrait"])expect(html).not.toContain(hidden)});

  it("presents online-only browse cards without physical-location wording",async()=>{const store={public_ref:"WNZ-ONLINE-CARD",slug:"online-card",name:"Online card",description:"Public description",category:null,store_type:"online_only",country_code:"CD",city:"Kinshasa",area:"Hidden area",commune:"Hidden commune",quartier:null,address:"Hidden address",landmark:null,public_detail_url:"https://nihiloba.com/shida/wenze/online-card",whatsapp_url:null,fulfillment:{methods:["delivery"],delivery:{fee_type:"free",fee:null,currency:null,areas:["Gombe"]}},owner_phone:"PRIVATE"};vi.stubGlobal("fetch",vi.fn().mockResolvedValue(new Response(JSON.stringify({items:[store],count:1}))));const html=renderToStaticMarkup(await WenzeCollection({l:"en",search:{}}));expect(html).toContain("Online shop");expect(html).toContain("https://nihiloba.com/shida/wenze/online-card");expect(html).not.toContain("Hidden area");expect(html).not.toContain("Hidden commune");expect(html).not.toContain("Hidden address");expect(html).not.toContain("PRIVATE")});

  it("uses a neutral label and hides address assumptions for legacy stores",async()=>{const store={public_ref:"WNZ-LEGACY-UI",slug:"legacy-ui",name:"Legacy shop",description:null,category:null,country_code:"CD",city:"Kinshasa",area:"Legacy area",commune:"Legacy commune",quartier:null,address:"Legacy address",landmark:null,products:[],public_detail_url:"https://nihiloba.com/shida/wenze/legacy-ui",whatsapp_url:null};vi.stubGlobal("fetch",vi.fn().mockResolvedValue(new Response(JSON.stringify(store))));const html=renderToStaticMarkup(await WenzeStorePage({l:"en",id:"legacy-neutral-render-test"}));expect(html).toContain("<p class=\"wenze-store-type\">Shop</p>");expect(html).not.toContain("Legacy area");expect(html).not.toContain("Legacy commune");expect(html).not.toContain("Legacy address");expect(html).not.toContain("Pickup available")});

  it("supports Wenze store and product route entry points with public references",async()=>{const store={public_ref:"WNZ_DIRECT_1",slug:"direct-store",name:"Direct store",description:null,category:null,store_type:"online_only",country_code:null,city:null,area:null,commune:null,quartier:null,address:null,landmark:null,products:[],public_detail_url:"https://nihiloba.com/shida/wenze/direct-store",whatsapp_url:null,fulfillment:{methods:[],delivery:null}};const product={public_ref:"WNP_DIRECT_1",slug:"direct-product",name:"Direct product",description:null,category:null,price:null,currency:null,price_negotiable:false,available_stock:null,has_variants:false,variant_type:null,variants:[],images:[],public_detail_url:"https://nihiloba.com/shida/wenze/products/direct-product",buy_url:null,store:null,fulfillment:{methods:[],delivery:null}};const fetchMock=vi.fn().mockResolvedValueOnce(new Response(JSON.stringify(store))).mockResolvedValueOnce(new Response(JSON.stringify(product)));vi.stubGlobal("fetch",fetchMock);const storeEntry=await WenzeStoreRoute({params:Promise.resolve({store:"WNZ_DIRECT_1"})}),productEntry=await WenzeProductRoute({params:Promise.resolve({product:"WNP_DIRECT_1"})});expect(storeEntry.props).toMatchObject({l:"en",id:"WNZ_DIRECT_1"});expect(productEntry.props).toMatchObject({l:"en",id:"WNP_DIRECT_1"});const storeHtml=renderToStaticMarkup(await WenzeStorePage({l:"en",id:"WNZ_DIRECT_1"})),productHtml=renderToStaticMarkup(await WenzeProductPage({l:"en",id:"WNP_DIRECT_1"}));expect(storeHtml).toContain("Direct store");expect(productHtml).toContain("Direct product");expect(fetchMock.mock.calls[0][0]).toContain("/wenze/stores/WNZ_DIRECT_1");expect(fetchMock.mock.calls[1][0]).toContain("/wenze/products/WNP_DIRECT_1")});
