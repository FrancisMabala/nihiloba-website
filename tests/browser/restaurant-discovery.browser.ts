...
for (const width of [390, 768, 1440]) test(`editorial discovery at ${width}px`, async ({ page }) => {
 ...
 expect(band.width).toBe(Math.min(width, 720)); expect(band.height).toBeCloseTo(band.width * 724 / 2171, 1);
 ...
 await page.screenshot({ path: `.s3a-local/terrasse-quartier-${width}.png`, fullPage: true });
});
...
test("bounded responsive banner requests without the master PNG", async ({ page }) => {
 ...
 for (const kind of ["cold", "warm"]) {
  ...
  expect(images.filter(path => path.includes("terrasse-quartier"))).toEqual(["/images/restaurants/terrasse-quartier-480.webp"]);
  expect(images.some(path => path.includes("table-partagee"))).toBe(false);
