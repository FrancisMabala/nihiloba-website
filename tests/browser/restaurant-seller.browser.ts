...
test("selection, pagination, private draft and changed-field profile edit", async ({ page }) => {
 ...
 expect(s.writes[0].body?.operation_key).toBeTruthy();
...
test("unit, configured amount and unknown pricing retain exact values", async ({ page }) => {
 const s = await fixture(page); await open(page); await page.getByRole("button", { name: "Dishes & components", exact: true }).click();
 for (const model of ["UNIT_PRICED", "AMOUNT_PRICED", "UNKNOWN"]) {
  await page.getByRole("button", { name: "Add another", exact: true }).click(); await page.getByLabel("Name", { exact: true }).fill("Fixture dish"); await page.getByRole("combobox", { name: "Category", exact: true }).selectOption("RMC_fixture"); await page.getByRole("combobox", { name: "Pricing", exact: true }).selectOption(model);
  ...
  await page.getByRole("button", { name: "Save changes", exact: true }).click(); await expect(page.getByRole("button", { name: "Add another", exact: true })).toBeVisible();
 }
...
test("hours unknown versus closed, publication and exact QR destination", async ({ page }) => {
 const s = await fixture(page); await open(page); await page.getByRole("button", { name: "Hours & closures", exact: true }).click(); await expect(page.getByText("Unknown · Africa/Kinshasa", { exact: true })).toBeVisible();
 ...
 await page.getByRole("button", { name: "Profile", exact: true }).click(); await page.getByRole("button", { name: "Publish", exact: true }).click(); await page.getByLabel("I confirm these changes", { exact: true }).check(); await page.getByRole("button", { name: "Save changes", exact: true }).click();
  await page.getByRole("button", { name: "Prepare menu link & QR", exact: true }).click(); await expect(page.getByRole("region", { name: "Restaurants & Malewa" }).getByRole("link", { name: "Open on WhatsApp" })).toHaveAttribute("href", "https://api.nihiloba.com/go/fixture_only"); await expect(page.getByRole("img", { name: "Menu QR code" })).toBeVisible();
...
test("expired offering copy stays unconfirmed until a separate confirmation", async ({ page }) => {
 ...
 await page.getByRole("button", { name: "Copy to new dates", exact: true }).click(); await page.getByLabel("Starts at", { exact: true }).fill("2027-01-01T10:00:00+01:00"); await page.getByLabel("Ends at", { exact: true }).fill("2027-01-01T12:00:00+01:00"); await page.getByRole("button", { name: "Save changes", exact: true }).click(); await expect(page.getByText("Awaiting confirmation", { exact: true })).toBeVisible();
