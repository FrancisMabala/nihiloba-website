...
describe("SHIDA public client", () => {
  ...
  it("parses the Jobs collection, detail and employer contracts without retaining private fields",async()=>{const employer={public_ref:"EMP_test123",slug:"rawbank",name:"RAWBANK",profile_image:{url:"https://res.cloudinary.com/dbrxpvmzp/image/upload/shida/jobs/rawbank.png",alt:"RAWBANK"},city:"Kinshasa",area:"Gombe",owner_phone:"PRIVATE"},location={country_code:"CD",city:"Kinshasa",area:"Gombe",commune:"Gombe",quartier:null,display:"Gombe, Kinshasa",exact_address:"PRIVATE"},summary={public_ref:"JOB_test123",slug:"it-support-rawbank",title:"IT Support Engineer",employer,location,description_preview:"Support users and systems.",compensation:"500 USD",published_at:"2026-08-12T10:00:00",status:"open",public_url:"https://nihiloba.com/shida/emplois/it-support-rawbank",apply_url:"https://wa.me/123?text=opaque",apply_label:"Apply on SHIDA",poster_id:99,recruiter_phone:"PRIVATE"},detail={...summary,description:"Full public description",requirements_document:{available:true,url:null,storage_path:"PRIVATE"},public_social_link:"https://www.linkedin.com/company/rawbank",candidate_phone:"PRIVATE"},profile={...employer,description:"Public employer description",open_job_count:1,jobs:[summary],business_owner_id:7};const collection={items:[summary],pagination:{page:2,page_size:12,total_items:13,total_pages:2},candidate_names:["PRIVATE"]};const fetchMock=vi.fn().mockResolvedValueOnce(new Response(JSON.stringify(collection))).mockResolvedValueOnce(new Response(JSON.stringify(detail))).mockResolvedValueOnce(new Response(JSON.stringify(profile)));vi.stubGlobal("fetch",fetchMock);const jobs=await getJobs({query:"support",city:"Kinshasa",commune:"Gombe",page:2,page_size:12});expect(jobs.pagination).toEqual({page:2,page_size:12,total_items:13,total_pages:2});expect(jobs.items[0].public_ref).toBe("JOB_test123");expect(jobs.items[0].application).toEqual({mode:null,available:false,capability:"none",apply_url:null,external_action:null});expect(jobs.items[0].application_mode).toBeNull();expect(jobs.items[0].application_available).toBe(false);expect(jobs.items[0].salary_visibility).toBe("not_provided");expect(jobs.items[0].external_verified).toBe(false);expect(jobs.items[0].organization_verified).toBe(false);expect(jobs.items[0].employer.identity_type).toBe("unknown");expect(jobs.items[0]).not.toHaveProperty("poster_id");expect(jobs.items[0].employer).not.toHaveProperty("owner_phone");expect(jobs.items[0].location).not.toHaveProperty("exact_address");const parsedJob=await getJob("job-contract-test");expect(parsedJob.requirements_document).toEqual({available:true,url:null});expect(parsedJob).not.toHaveProperty("candidate_phone");const parsedEmployer=await getJobEmployer("employer-contract-test");expect(parsedEmployer.jobs[0].title).toBe("IT Support Engineer");expect(parsedEmployer).not.toHaveProperty("business_owner_id");expect(fetchMock.mock.calls[0][0]).toContain("query=support&city=Kinshasa&commune=Gombe&page=2&page_size=12");});

  it("accepts the backend individual employer identity without adding trust",async()=>{const summary={public_ref:"JOB_individual",slug:"JOB_individual",title:"Garde enfants",offer_type:"individual",origin:"direct",employer:{public_ref:"EMP_private",slug:null,name:"Private employer",profile_image:null,city:null,area:null,identity_type:"individual"},location:{country_code:null,city:null,area:null,commune:"Bandal",quartier:null,display:"Bandal, [private information hidden]"},compensation:"50 EUR",salary:null,salary_visibility:"not_provided",published_at:"2026-07-16T19:45:06Z",status:"open",lifecycle_state:"published",public_url:"https://nihiloba.com/shida/emplois/JOB_individual",application:{mode:"shida",available:true,capability:"shida_application",apply_url:"https://wa.me/46769709059?text=SHIDA%20opaque",external_action:null},apply_label:"Apply on SHIDA",external_verified:false,organization_verified:false};vi.stubGlobal("fetch",vi.fn().mockResolvedValue(new Response(JSON.stringify({items:[summary],pagination:{page:1,page_size:12,total_items:1,total_pages:1}}))));const parsed=(await getJobs()).items[0];expect(parsed.employer.identity_type).toBe("individual");expect(parsed.external_verified).toBe(false);expect(parsed.organization_verified).toBe(false);});
