...
describe("SHIDA public client", () => {
  ...
  it("parses public Wenze stores and products without retaining seller PII", async () => {
    ...
    const parsed=await getWenzeProduct("dress-test");expect(parsed.store?.name).toBe("Mado");expect(parsed.buy_url).toBe("https://wa.me/product");expect(parsed.variants[0].buy_url).toBe("https://wa.me/variant-m");expect(parsed.variants[1]).toMatchObject({label:"L",available_stock:0,is_available:false,buy_url:null});
  });
});
