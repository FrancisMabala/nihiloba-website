...
import type {
  ...
  ServiceCollection, ServiceLocation, ServiceOffering, ServiceRating, ServiceSearch,
...
function serviceOffering(value:unknown):ServiceOffering { const item=record(value); return {name:text(item.name,true)!,duration_minutes:integer(item.duration_minutes),price:text(item.price),currency:text(item.currency),description:text(item.description)}; }
...
export async function getServiceAvailability(key:string,from:string,to:string):Promise<ServiceAvailability> { const params=new URLSearchParams({from,to});return serviceAvailability(await request(`/api/public/shida/services/${encodeURIComponent(key)}/availability?${params}`,false)); }
