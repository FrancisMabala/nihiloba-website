...
import type {
  ...
  PublicCollection,
  PublicImage,
  JobCollection, JobSearch, PublicEntityActions, PublicJob, PublicJobApplication, PublicJobEmployer, PublicJobEmployerSummary, PublicJobExperience, PublicJobExternalAction, PublicJobLifecycle, PublicJobLocation, PublicJobSalary, PublicJobSalaryVisibility, PublicJobSummary,
...
function jobSalary(value:unknown):PublicJobSalary|null { if(value==null)return null;const item=record(value);return {minimum:text(item.minimum),maximum:text(item.maximum),currency:text(item.currency),period:text(item.period)}; }
function jobSalaryVisibility(value:unknown):PublicJobSalaryVisibility { const parsed=text(value)??"not_provided";if(parsed!=="visible"&&parsed!=="hidden"&&parsed!=="not_provided")throw new ShidaApiError("malformed");return parsed; }
function jobExternalAction(value:unknown):PublicJobExternalAction|null { if(value==null)return null;const item=record(value),mode=text(item.mode,true),action=text(item.action,true),endpoint=text(item.endpoint,true)!;if((mode!=="redirect"&&mode!=="instructions")||action!==mode||!/^\/api\/public\/shida\/jobs\/JOB_[A-Za-z0-9_-]+\/external-action$/.test(endpoint))throw new ShidaApiError("malformed");return {mode,action,endpoint}; }
function jobApplication(item:Record<string,unknown>):PublicJobApplication { if(item.application==null)return {mode:null,available:false,capability:"none",apply_url:null,external_action:null};const value=record(item.application),mode=text(value.mode),available=flag(value.available),capability=text(value.capability,true),externalAction=jobExternalAction(value.external_action);if(mode!==null&&mode!=="shida"&&mode!=="redirect"&&mode!=="email_assisted"&&mode!=="instructions")throw new ShidaApiError("malformed");if(capability!=="shida_application"&&capability!=="external_redirect"&&capability!=="external_instructions"&&capability!=="none")throw new ShidaApiError("malformed");return {mode,available,capability,apply_url:text(value.apply_url),external_action:externalAction}; }
function jobExperience(value:unknown):PublicJobExperience { if(value==null)return {text:null,minimum_years:null,requirement_kind:"unspecified"};const item=record(value),kind=text(item.requirement_kind)??"unspecified";if(kind!=="required"&&kind!=="preferred"&&kind!=="approximate"&&kind!=="not_required"&&kind!=="unspecified")throw new ShidaApiError("malformed");return {text:text(item.text),minimum_years:integer(item.minimum_years),requirement_kind:kind}; }
function jobSummary(value:unknown):PublicJobSummary {
  const item=record(value),offerType=item.offer_type??"individual",origin=item.origin??"direct",relationships=record(item.relationship_capabilities??{can_save:true,save_target_type:"job",save_target_ref:item.public_ref,can_share:true,can_follow_employer:false,follow_target_type:null,follow_target_ref:null}),status=lifecycle(item.status),state=lifecycle(item.lifecycle_state??item.status);
  if((offerType!=="enterprise"&&offerType!=="individual")||(origin!=="direct"&&origin!=="external")||!Array.isArray(item.capabilities??[]))throw new ShidaApiError("malformed");
  const canSave=relationships.can_save,canShare=relationships.can_share,canFollow=relationships.can_follow_employer;
  if(typeof canSave!=="boolean"||typeof canShare!=="boolean"||typeof canFollow!=="boolean")throw new ShidaApiError("malformed");
  const application=jobApplication(item),salary=jobSalary(item.salary),salaryVisibility=jobSalaryVisibility(item.salary_visibility),followTargetType=text(relationships.follow_target_type);
  if(canFollow&&followTargetType!=="organization_directory"&&followTargetType!=="company")throw new ShidaApiError("malformed");
  return {public_ref:publicRef(item.public_ref,"JOB"),slug:text(item.slug),title:text(item.title,true)!,offer_type:offerType,origin,capabilities:((item.capabilities??[]) as unknown[]).map((value)=>text(value,true)!),professional_category:text(item.professional_category),category:text(item.category),vacancies:integer(item.vacancies),contract_type:text(item.contract_type),contract_type_raw:text(item.contract_type_raw),seniority:text(item.seniority),work_mode:text(item.work_mode),work_mode_raw:text(item.work_mode_raw),work_schedule:text(item.work_schedule),application_deadline:text(item.application_deadline),employer:jobEmployer(item.employer),location:jobLocation(item.location),short_summary:text(item.short_summary),summary_preview:text(item.summary_preview),description_preview:text(item.description_preview),compensation:text(item.compensation),salary,salary_visibility:salaryVisibility,published_at:text(item.published_at),status,lifecycle_state:state,public_url:text(item.public_url,true)!,application,application_mode:application.mode,apply_url:application.apply_url,apply_label:text(item.apply_label,true)!,deadline_label:text(item.deadline_label),relationship_capabilities:{can_save:canSave,save_target_type:"job",save_target_ref:publicRef(relationships.save_target_ref,"JOB"),can_share:canShare,can_follow_employer:canFollow,follow_target_type:canFollow?followTargetType as "organization_directory"|"company":null,follow_target_ref:canFollow?publicRef(relationships.follow_target_ref):null},external_application_action:application.external_action,external_verified_label:text(item.external_verified_label),external_verified:item.external_verified===true,organization_verified_label:text(item.organization_verified_label),organization_verified:item.organization_verified===true,application_available:application.available};
}
function job(value:unknown):PublicJob { const item=record(value),requirements=record(item.requirements_document),source=item.external_source==null?null:record(item.external_source);if(requirements.url!==null||typeof requirements.available!=="boolean")throw new ShidaApiError("malformed");let externalSource:PublicJob["external_source"]=null;if(source){const mode=text(source.application_mode,true);if(mode!=="redirect"&&mode!=="instructions"&&mode!=="email_assisted")throw new ShidaApiError("malformed");externalSource={explanation:text(source.explanation,true)!,canonical_url:text(source.canonical_url),source_label:text(source.source_label,true)!,application_mode:mode,application_instructions:text(source.application_instructions),email_assisted_available:source.email_assisted_available===true};}return {...jobSummary(value),description:text(item.description),description_fallback:text(item.description_fallback),workplace_reference:text(item.workplace_reference),start_date:text(item.start_date),responsibilities:text(item.responsibilities),qualifications:text(item.qualifications),experience:jobExperience(item.experience),education:text(item.education),skills:text(item.skills),personal_qualities:text(item.personal_qualities),languages:text(item.languages),benefits:text(item.benefits),employment_conditions:text(item.employment_conditions),requirements_document:{available:requirements.available,url:null},public_social_link:text(item.public_social_link),external_source:externalSource}; }
...
export async function request(path: string, revalidate: number | false): Promise<unknown> {
...
export function parseJobSearchParams(value:ApartmentRawSearchParams):JobSearch { const result:JobSearch={};for(const key of jobSearchKeys){const parsed=firstQueryValue(value[key]);if(parsed)result[key]=parsed;}const view=firstQueryValue(value.view);if(view==="professional"||view==="individual")result.view=view;const page=numericQuery(value.page,{integer:true,min:1,max:100_000}),size=numericQuery(value.page_size,{integer:true,min:1,max:50});if(page!=null)result.page=page;if(size!=null)result.page_size=size;return result; }
export function jobSearchQuery(search:JobSearch):string { const params=new URLSearchParams();for(const key of jobSearchKeys){const value=search[key]?.trim();if(value)params.set(key,value.slice(0,120));}if(search.view==="professional"||search.view==="individual")params.set("view",search.view);if(search.page&&Number.isInteger(search.page)&&search.page>0)params.set("page",String(search.page));if(search.page_size&&Number.isInteger(search.page_size)&&search.page_size>0&&search.page_size<=50)params.set("page_size",String(search.page_size));return params.size?`?${params}`:""; }
...
export async function getJobs(search:JobSearch={},language:"en"|"fr"="en"):Promise<JobCollection> { const {view:_,...apiSearch}=search;void _;const query=new URLSearchParams(jobSearchQuery(apiSearch).slice(1));query.set("language",language);return jobCollection(await request(`/api/public/shida/jobs?${query}`,60)); }
