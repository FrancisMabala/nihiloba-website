...
export function ImageGallery({ images, title, fallback, photosLabel, preload = false, sizes = "(max-width: 720px) calc(100vw - 32px), 1180px" }: { images: PublicImage[]; title: string; fallback: string; photosLabel: string; preload?: boolean; sizes?: string }) {
  ...
  return <section className="marketplace-gallery" aria-label={photosLabel}>
    <div className="marketplace-gallery-main">
      <MarketplaceImage src={active.url} alt={active.alt || `${title} — ${activeIndex + 1}`} fallback={fallback} sizes={sizes} preload={preload}/>
