"use client";
import { PrivateMenuPreview } from './restaurant-projection';
...
import { sellerDesignCopy } from "@/app/lib/restaurant-seller-design-copy";
...
import { RestaurantCounter } from './restaurant-counter';
import { RestaurantIntake } from './restaurant-intake';
import { RestaurantActivity } from './restaurant-activity';
import { RestaurantMenuBatch } from './restaurant-menu-batch';
import { leaveWork } from '@/app/lib/restaurant-work';
...
export function RestaurantWorkspace({ binding, locale }: { binding: string; locale: RestaurantLocale }) {
  ...
  const wt = (key:string) => workText(locale,key);
  ...
  const workFailure = useCallback((caught:unknown) => error(caught), []);
  ...
  async function select(ref: string, initialTab = "items") {
    ...
    if (alive.current) { setSelected(value); setTab(initialTab); setRows(menu?.result.items ?? []); setRowPage(1); setRowTotal(menu?.result.total ?? 0); setHours(null); setPreview(null); }
  }
  async function loadTab(next: string, nextPage = 1) {
    ...
    if (["orders","counter","intake","activity","batch"].includes(next)) { setTab(next); return; }
  ...
  function input(key: string, required = false) {
    ...
    return <label key={key}>{t(key)}{boolKeys.includes(key) ? <input type="checkbox" checked={value === "true"} required={required} onChange={event => change(key, String(event.target.checked))} /> : choicesForKey ? <select required={required} value={value} onChange={event => change(key, event.target.value)}><option value="">—</option>{choicesForKey.map(row => <option key={row.public_ref} value={row.public_ref}>{row.name}</option>)}</select> : key === 'timezone_name' ? <select required={required} value={value} onChange={event=>change(key,event.target.value)}><option value="">&mdash;</option>{[...new Set([value,...Intl.supportedValuesOf('timeZone')])].filter(Boolean).map(zone=><option key={zone} value={zone}>{zone.split('/').slice(1).join(' / ').replaceAll('_',' ')} ({zone.split('/')[0]})</option>)}</select> : options[key] ? <select required={required} value={value} onChange={event => change(key, event.target.value)}><option value="">—</option>{options[key].map(option => <option key={option} value={option}>{t(option)}</option>)}</select> : ["description", "opening_information", "service_modes"].includes(key) ? <textarea required={required} maxLength={4000} value={value} onChange={event => change(key, event.target.value)} /> : ["starts_at", "ends_at"].includes(key) ? <LocalTime value={value} timezone={selected?.profile.timezone_name ?? null} locale={locale} required={required} onChange={v=>change(key,v)}/> : <input required={required} name={key} type={key.includes("url") ? "url" : "text"} inputMode={["unit_price", "minimum_amount"].includes(key) ? "decimal" : "text"} maxLength={key === "name" ? 200 : 2000} value={value} onChange={event => change(key, event.target.value)} />}</label>;
  }
  ...
  return <section className="rst container" aria-label={t("restaurants")}>
    ...
    {editor ? <form onSubmit={submit} className={`rst-card rst-editor${editor.kind === "items" ? " rst-portion-editor" : ""}`} aria-label={t(editor.kind)}>
      ...
      {editor.kind === "items" && <p>{design.afterSave}</p>}
      ...
      <fieldset disabled={busy || Boolean(editor.operation) || editor.blocked} className="rst-fields">
        ...
        {editor.kind === "items" && <><div className="rst-wide">{input("name", true)}</div><fieldset className="rst-pricing-model rst-wide"><legend>{design.sell}</legend>{[["UNIT_PRICED", design.unit], ["AMOUNT_PRICED", design.amount], ["UNKNOWN", design.unknown]].map(([value, label]) => <label key={value}><input type="radio" name="pricing_model" checked={editor.fields.pricing_model === value} onChange={() => change("pricing_model", value)}/>{label}</label>)}</fieldset>{editor.fields.pricing_model !== "UNKNOWN" && input("currency", true)}{editor.fields.pricing_model === "UNIT_PRICED" && <>{input("sale_unit_label", true)}{input("unit_price", true)}</>}{editor.fields.pricing_model === "AMOUNT_PRICED" && <div className="rst-amounts"><p>{t("allowed_amounts")}</p>{(editor.fields.allowed_amounts || "").split(/;\s*/).map((amount, index, amounts) => <div className="rst-amount-row" key={index}><label><span className="sr-only">{t("allowed_amounts")} {index + 1}</span><input required inputMode="decimal" value={amount} onChange={event => change("allowed_amounts", amounts.map((item, i) => i === index ? event.target.value : item).join(";"))}/></label><span>{editor.fields.currency}</span><button type="button" aria-label={`${t("remove")} ${index + 1}`} disabled={amounts.length === 1} onClick={() => change("allowed_amounts", amounts.filter((_, i) => i !== index).join(";"))}>×</button></div>)}<button type="button" className="rst-text-button" onClick={() => change("allowed_amounts", `${editor.fields.allowed_amounts || ""};`)}>+ {design.amounts}</button><p className="rst-field-help">{design.portionHelp}</p>{input("minimum_amount")}</div>}{input("availability", true)}{input("visible")}<details className="rst-wide rst-extra-fields" open={editor.method === "POST" || !editor.fields.category_ref}><summary>{design.extras}</summary><div className="rst-fields">{input("category_ref", true)}{input("presentation", true)}{input("description")}{input("image_url")}{input("permanent")}</div></details></>}
        ...
        {editor.kind === "hours" && <div className="rst-wide"><p>{t("hoursHelp")}</p>{input("known")}<h3>{t("windows")}</h3>{editor.fields.known === "true" && <>{editor.windows.map((window, index) => <div className="rst-row" key={index}><label>{t("weekday")}<select value={window.weekday} onChange={event => changeHours({ windows: editor.windows.map((w, i) => i === index ? { ...w, weekday: Number(event.target.value) } : w) })}>{weekdays.map((day, i) => <option key={day} value={i}>{t(day)}</option>)}</select></label>{(["start", "end"] as const).map(key => <label key={key}>{t(key)}<input type="time" required value={window[key]} onChange={event => changeHours({ windows: editor.windows.map((w, i) => i === index ? { ...w, [key]: event.target.value } : w) })} /></label>)}{window.end <= window.start && <span>{wt('overnight')}</span>}{button("remove", () => changeHours({ windows: editor.windows.filter((_, i) => i !== index) }))}</div>)}{button("addWindow", () => changeHours({ windows: [...editor.windows, { weekday: 0, start: "09:00", end: "17:00" }] }))}</>}<p>{wt('splitHelp')}</p><h3>{t("closures")}</h3>{editor.closures.map((closure, index) => <div className="rst-row" key={index}>{closure.confirmed_at ? <p>{localInstant(closure.starts_at, closure.timezone_name ?? null)} — {localInstant(closure.ends_at, closure.timezone_name ?? null)}</p> : (["starts_at", "ends_at"] as const).map(key => <label key={key}>{t(key)}<LocalTime value={closure[key]} timezone={selected?.profile.timezone_name ?? null} locale={locale} onChange={value => changeHours({ closures: editor.closures.map((c, i) => i === index ? { ...c, [key]: value } : c) })} /></label>)}{button("remove", () => changeHours({ closures: editor.closures.filter((_, i) => i !== index) }))}</div>)}{button("addClosure", () => changeHours({ closures: [...editor.closures, { starts_at: "", ends_at: "" }] }))}{input("confirm", true)}</div>}
      ...
      </div>{editor.kind === "items" && <aside className="rst-portion-help"><h3>{design.portion}</h3><p>{design.portionHelp}</p><p>{design.afterSave}</p></aside>}
    </form> : <>
      {!selected ? <><div className="rst-actions"><label>{t("establishments")}<select value={filter} disabled={busy} onChange={event => void run(() => loadList(1, event.target.value))}><option value="">{t("all")}</option>{["draft", "active", "inactive"].map(status => <option key={status} value={status}>{t(status)}</option>)}</select></label>{list?.actions?.includes("create") && button("add", () => void run(() => open("create")))}{button("reload", () => void run(() => loadList()))}</div><div className="rst-grid">{list?.items.map(row => <article className="rst-card" key={row.public_ref}><h2>{row.profile.name}</h2><p>{t(row.status)}</p>{button("edit", () => void run(() => select(row.public_ref)))}</article>)}</div>{list && !list.items.length && <p>{t("empty")}</p>}<div className="rst-actions">{button("previous", () => void run(() => loadList(page - 1)), page <= 1)}{button("next", () => void run(() => loadList(page + 1)), !list || page * 20 >= list.total)}</div></> : <>
        ...
        {tab === 'activity' && <RestaurantActivity key={path} path={path} binding={binding} locale={locale} timezone={selected.profile.timezone_name} onFailure={workFailure}/>}
        {tab === 'batch' && <RestaurantMenuBatch key={path} path={path} binding={binding} locale={locale} onFailure={workFailure}/>}
        ...
        {tab === "sharing" && (selected.actions.includes("links") ? <RestaurantSharing key={selected.public_ref} establishment={selected.public_ref} binding={binding} locale={locale} onFailure={error}/> : <p>{t("missing")}: {selected.missing_publication_fields.map(t).join(", ") || t(selected.status)}</p>)}
        {tab === "preview" && <article className="rst-card"><h2>{t("preview")}</h2><p>{t("previewNote")}</p>{preview && details(preview, ["name", "type_label", "description", "location", "opening_status", "opening_information", "service_modes"])}<PrivateMenuPreview key={`${path}:${binding}:${locale}`} path={path} binding={binding} locale={locale} onFailure={workFailure}/></article>}
...
{["categories", "items", "offerings"].includes(tab) && <>{tab === "offerings" && <div className="rst-menu-heading"><h2>{design.daily}</h2><button type="button" className="rst-text-button" disabled={busy} onClick={() => void run(() => open("offerings"))}>+ {design.add}</button></div>}<div className="rst-menu-rows">{rows.map(row => <article className="rst-menu-row" key={row.public_ref}><div><h2>{row.name ?? row.item?.name}</h2>{tab === "items" && <p>{row.pricing_model === "UNIT_PRICED" ? `${design.unit} · ${display(row.sale_unit_label)}` : row.pricing_model === "AMOUNT_PRICED" ? design.amount : design.unknown}</p>}</div>{tab === "items" ? <><p className="rst-price">{row.pricing_model === "UNIT_PRICED" ? `${display(row.unit_price)} ${display(row.currency)} / ${display(row.sale_unit_label)}` : row.pricing_model === "AMOUNT_PRICED" ? `${display(row.allowed_amounts)} ${display(row.currency)}` : design.unknown}</p><div><p className={`rst-availability rst-state-${row.availability}`}>{t(String(row.availability ?? "unknown"))}</p>{row.visible === false && <p>{t("hidden")}</p>}</div></> : details(row, tab === "categories" ? ["visible"] : ["state", "visible", "availability"])}{tab === "offerings" && <p>{localInstant(row.starts_at, String(row.timezone_name))} — {localInstant(row.ends_at, String(row.timezone_name))}</p>}<div className="rst-actions">{button("edit", () => void run(() => open(tab as Kind, row)))}{tab === "offerings" && button("copy", () => void run(() => open("copy", row)))}</div></article>)}</div>{!rows.length && <p className="rst-empty">{t("empty")}</p>}<div className="rst-actions rst-pagination">{button("previous", () => void run(() => loadTab(tab, rowPage - 1)), rowPage <= 1)}<span>{design.page} {rowPage}</span>{button("next", () => void run(() => loadTab(tab, rowPage + 1)), rowPage * 20 >= rowTotal)}</div></>}
      </>}
    </>}
  </section>;
}
