...
export function ReceiptCard({order,locale}:{order:Order;locale:RestaurantLocale}) {const t=(k:string)=>workText(locale,k);const zone=order.receipt_terms?.timezone_name ?? order.receipt_terms?.pickup_window?.timezone_name ?? order.receipt_terms?.arrival_window?.timezone_name ?? 'UTC';return <article className="rst-card"><h2>{t('receipt')} · {order.receipt_terms?.establishment_name}</h2><p>{order.order_ref}</p><p>{t('current')}: {t(order.state)} · {t(order.fulfillment_method)}</p><p>{dateText(order.submitted_at,locale,zone)} · {zone.split('/').at(-1)?.replaceAll('_',' ')}</p>{order.response_deadline && <p>{t('deadline')}: {dateText(order.response_deadline,locale,zone)}</p>}{order.receipt_terms && <FoodTerms terms={order.receipt_terms} locale={locale}/>}<p>{t('payment')}</p></article>;}
