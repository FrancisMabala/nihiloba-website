...
export function RestaurantOrderScreen({path,binding,locale,timezone,onFailure}:{path:string;binding:string;locale:RestaurantLocale;timezone:string|null;onFailure:(e:unknown)=>void}) {
 ...
 <div className="rst-order-layout"><div>{list && <><p>{list.total} · {t('orders')}</p>{!list.items.length && !status && <p>{t('empty')}</p>}{list.items.map(o=><article className="rst-card" key={o.order_ref}><h3>{o.order_ref}</h3><p>{t(o.state)} · {t(o.fulfillment_method)}</p><p>{o.order_total} {o.currency}</p><p>{dateText(o.submitted_at,locale,timezone)}</p>{o.cancellation_pending && <p>{t('cancellation')}</p>}<button type="button" className="button button-secondary" disabled={blocked || !!api.pending} onClick={()=>void open(o)}>{t('review')}</button></article>)}<div className="rst-actions"><button type="button" disabled={page===1 || blocked} onClick={()=>{setPage(n=>n-1);setStatus('loading');}}>{t('previous')}</button><span>{page}</span><button type="button" disabled={page*20>=list.total || blocked} onClick={()=>{setPage(n=>n+1);setStatus('loading');}}>{t('next')}</button></div></>}</div>
 {detail && <article className="rst-card"><h2>{detail.order_ref}</h2><p>{t('current')}: {t(detail.state)} · {t(detail.fulfillment_method)}</p>{detail.response_deadline && <p>{t('deadline')}: {dateText(detail.response_deadline,locale,timezone)}</p>}{detail.cancellation_pending && <p>{t('cancellation')}</p>}{detail.receipt_terms && <FoodTerms terms={detail.receipt_terms} locale={locale}/>}<Link prefetch={false} href={receiptPath}>{t('receipt')}</Link>
