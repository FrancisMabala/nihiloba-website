  ...
  return (
    <footer className="site-footer">
      <div className="container footer-grid">
        ...
        <div className="footer-column"><p className="footer-label">{t.legal}</p><Link href={localizedPath(locale,"/privacy")}>{t.privacy}</Link><Link href={locale==="en"?"/en/data-protection":"/fr/protection-des-donnees"}>{t.dataProtection}</Link><Link href={locale==="en"?"/en/security":"/fr/securite"}>{t.security}</Link><Link href={localizedPath(locale,"/terms")}>{t.terms}</Link></div>
      </div>
